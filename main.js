...
const newDino = () => {
    let dinoId = `dino${dinos.length + 1}`;
    let dinoName = document.getElementById("dino-name").value;
    let dinoType = document.getElementById("dino-type").value;
    let dinoAge = document.getElementById("dino-age").value;
    let dinoOwner = document.getElementById("dino-owner").value;
    let dinoImage = document.getElementById("dino-pic").value;
    let checkifEmpty = [dinoName, dinoType, dinoAge, dinoOwner, dinoImage].some(isEmpty);
    if (!checkifEmpty) {
        newDinoObject(dinoId, dinoName, dinoType, dinoAge, dinoOwner, dinoImage);
        formCollapse();
    }
}

const events = () => {
    document.getElementById("btn-submit").addEventListener("click", newDino);
}
